                ...
                case "reddit":
                    ...
                    function ExtractRedditUrl(body, tries) {
                        if (tries >= 10)
                        {
                            ...
                            return reject({reason: "retry limit exceeded", message: "Failed to find a suitable post", subbredit: site});
                        }
                        tries++;
                        // grabs a random post
                        let post = body[Math.floor(Math.random() * body.length)].data;
                        // checks if the post url ends with an image extension
                        switch ((/(\.jpg|\.png|\.gif|\.jpeg|\.gifv)$/ig).test(post.url) || post.url.includes(".com") || post.url.includes("external-preview.redd.it")) {
                            case true:
                                if(post.url.includes("reddit.com/gallery"))
                    ...
                    let sortBy = ["top", "hot"], filter = sortBy[Math.floor(Math.random() *sortBy.length)];
                    let url = `https://reddit.com/r/${site}/${filter}.json?limit=45`;
                    if(filter == "top")
                    {
                        let options = ["hour", "day", "week", "month", "year", "all"], filter2 = options[Math.floor(Math.random() *sortBy.length)];
                        url = `https://reddit.com/r/${site}/${filter}.json?t=${filter2}`;
                    }
